...
const Nav = () => {
    ...
    const {adapter, setReadyState, setAccount, setNetwork, readyState} = useGlobalContext()
    useEffect(() => {
        console.log(adapter)
            ...
            <div className="flex items-center justify-between px-4 py-2 h-full">
                ...
                <nav className=" h-full">
                    <ul className="flex items-center space-x-4">
                        <Link href="/">
                            <li className={`${isActive("/") ? 'text-white font-semibold' : 'text-gray-300 hover:text-white'} h-full`}>Home</li>
                        </Link>
                        <Link href="/marketplace">
                            <li className={`${isActive("/marketplace") ? 'text-white font-semibold' : 'text-gray-300 hover:text-white'} h-full`}>Marketplace</li>
                        </Link>
                        <Link href="/mytickets">
